...
The database is **Cloud Firestore**, a document store. Five top-level
...
Two collections use a **shared document id** instead of a foreign key: a
tournament's roster and its clock are stored under the tournament's own id. That
makes the one-to-one relationship structural rather than conventional — it is
impossible to create two clocks for one tournament.
...
erDiagram
    ...
    users {
        ...
        object_array tournaments "EMBEDDED results history"
    }
...
**`users.tournaments[]`** — one row per result in a player's history.
...
tournaments (id, auto)
  ...
  └── referenced by ─────────────► users.tournaments[].tournamentId
...
**Why a player's history is embedded rather than its own collection.** It is
always read as a whole and always scoped to one player, so embedding costs one
document read instead of a query. The trade-off is real and worth stating: the
...
reading every user document. At the scale this application targets, a home game
...
> stored — the fact rows live embedded in `users.tournaments[]` and
